        ...
        try:
            ...
            while retries > 0:
                try:
                    ...
                    # Keep a reference to the engine for tests to use directly
                    cls.postgres_engine = engine
                    conn = engine.connect()
                    conn.close()
            ...
            log.error(ex)
            cls.teardown_class()
            # Docker or DB not available in this environment; skip these integration tests instead of failing.
            pytest.skip(f"Could not setup test environment (skipping integration tests): {ex}")
    ...
    def teardown_method(self):
        ...
        for table in tables:
            try:
                ...
                # Fallback for SQLite or other DBs that do not support TRUNCATE
