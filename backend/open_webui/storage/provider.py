...
try:
    ...
    # Optional dependency - gracefully degrade if google-cloud-storage is not installed or partially installed
    storage = None
...
class LocalStorageProvider(StorageProvider):
    @staticmethod
    def upload_file(
        file: BinaryIO, filename: str, tags: Dict[str, str] | None = None
    ) -> Tuple[bytes, str]:
        contents = file.read()
        if not contents:
...
class S3StorageProvider(StorageProvider):
    ...
    def upload_file(
        self, file: BinaryIO, filename: str, tags: Dict[str, str] | None = None
    ) -> Tuple[bytes, str]:
...
class GCSStorageProvider(StorageProvider):
    def __init__(self):
        # Do not create the GCS client during import/instantiation. Tests may import
        # the class without having application default credentials available; lazily
    ...
    def upload_file(
        ...
        # Ensure client/bucket are ready (lazily initialize if necessary)
        self._ensure_client()
    ...
    def get_file(self, file_path: str) -> str:
        ...
        # Lazily initialize client and bucket
        self._ensure_client()
        try:
...
class AzureStorageProvider(StorageProvider):
    def __init__(self):
        # Lazily initialize Azure client; do not fail during import if env vars not present
    ...
    def upload_file(
        ...
        # Lazily initialize client and container
        self._ensure_client()
        contents, file_path = LocalStorageProvider.upload_file(file, filename, tags)
        try:
