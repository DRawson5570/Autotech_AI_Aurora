...
app.include_router(autodb.router, prefix="/api/v1/autodb", tags=["autodb"])
...
async def chat_completion(
    ...
    try:
        if user and getattr(user, "role", None) != "admin":
            ...
            if bal <= 0:
                # Check if user has an active paid subscription (overage allowed)
                sub = _billing.get_user_subscription(user.id)
                has_paid_tier = (
                    sub 
                    and getattr(sub, "status", None) == "active" 
                    and getattr(sub, "tier_id", None) not in (None, "none")
                )
                ...
                # Paid tier users can continue - overage will be billed at cycle end
