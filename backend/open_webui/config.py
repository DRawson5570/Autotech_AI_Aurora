...
# Billing
#
# Per-token USD rate used to price token purchases for end users.
# This is persisted via the DB-backed config system so admins can update pricing at runtime.
#
# Default matches existing env-based estimate used in billing admin exports.

BILLING_TOKEN_USD_RATE = PersistentConfig(
    "BILLING_TOKEN_USD_RATE",
    "billing.token_usd_rate",
    float(os.environ.get("BILLING_TOKEN_USD_RATE", "0.00002")),
)
...
GOOGLE_API_BASE_URL = os.environ.get(
...
# For Gemini 3 models: "minimal", "low", "medium", "high" (Flash) or "low", "high" (Pro)
# Default "low" for faster responses
GEMINI_THINKING_LEVEL = os.environ.get("GEMINI_THINKING_LEVEL", "low")

# For Gemini 2.5 models: 0 to disable, -1 for dynamic, or specific token budget
# Default 0 to disable thinking for faster responses
GEMINI_25_THINKING_BUDGET = int(os.environ.get("GEMINI_25_THINKING_BUDGET", "0"))

# Persistent storage for provider-level credentials (encrypted). This holds provider-specific
# encrypted API keys or other credentials so admin-set credentials survive restarts.
MODEL_PROVIDER_CREDENTIALS = PersistentConfig(
    "MODEL_PROVIDER_CREDENTIALS",
    "model_provider.credentials",
    {},
)
...
- If the context provides relevant data, use it directly to answer the query.
