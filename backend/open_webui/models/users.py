...
class UsersTable:
    ...
    def get_first_user(self) -> UserModel:
        """Return the first *admin* user by creation time.

        Previously this returned the earliest-created user overall which could
        be a non-admin (e.g., a test user). That caused primary-admin checks
        to misidentify a non-admin as the primary admin and prevented
        legitimate admin actions. Only consider users with role='admin'.
        """
        try:
            with get_db() as db:
                user = (
