...
class FilesTable:
    ...
    def get_file_by_hash_and_knowledge_id(
        self, hash: str, knowledge_id: str
    ) -> Optional[FileModel]:
        """Get a file by its content hash within a specific knowledge base.
        
        Checks the knowledge_file junction table to find files with the given hash
        that are already associated with the specified knowledge base.
        """
        from open_webui.models.knowledge import KnowledgeFile
        
        with get_db() as db:
            # Join File with KnowledgeFile to find files with this hash in this knowledge base
            result = (
                db.query(File)
                .join(KnowledgeFile, File.id == KnowledgeFile.file_id)
                .filter(File.hash == hash)
