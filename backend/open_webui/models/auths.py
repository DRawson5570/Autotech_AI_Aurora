    ...
    def authenticate_user(
        ...
        try:
            with get_db() as db:
                ...
                log.info(f"authenticate_user: auth raw object={auth}")
                if auth:
                    try:
                        result = verify_password(auth.password)
                        log.info(f"authenticate_user: auth found id={auth.id} verify_result={bool(result)}")
                    except Exception as e:
                        log.exception(f"authenticate_user: verification exception: {e}")
                        return None
                    ...
                    log.debug("authenticate_user: no auth record found for user")
                    return None
        except Exception:
            return None
