...
class KnowledgeTable:
    ...
    def is_file_in_knowledge(self, knowledge_id: str, file_id: str) -> bool:
        """Check if a file is already associated with a knowledge base."""
        with get_db() as db:
            existing = (
    ...
    def add_file_to_knowledge_by_id(
        ...
        with get_db() as db:
            # Check if file is already in this knowledge base
            existing = (
                db.query(KnowledgeFile)
                .filter_by(knowledge_id=knowledge_id, file_id=file_id)
                .first()
            ...
            if existing:
                log.warning(
                    f"File {file_id} is already in knowledge {knowledge_id}, skipping"
                )
