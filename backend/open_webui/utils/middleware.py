...
def ensure_system_greeting(form_data: dict, metadata: dict, user, request_id: str | None = None) -> dict:
    """If no system message exists, inject a deterministic greeting for the user.

    Returns the (possibly modified) form_data.
    """
    try:
        from open_webui.utils.misc import get_system_message

        if not get_system_message(form_data.get("messages", [])) and user:
            form_data = apply_system_prompt_to_body(
                "Hello {{USER_FIRST_NAME}}!", form_data, metadata, user
            )
            try:
                sys_msg = get_system_message(form_data.get("messages", []))
                preview = (sys_msg.get("content")[:120] if sys_msg else "<no-system-message>")
                uid = request_id[:8] if request_id else "unknown"
                user_id_val = user.id if hasattr(user, "id") else (user.get("id") if isinstance(user, dict) else None)
                log.info(f"injected_greeting: req={uid}, user={str(user_id_val)[:8] if user_id_val else 'unknown'}, preview={preview}")
            except Exception:
                pass
    except Exception:
        pass
...
# Global store for Mitchell images by message_id (for injection after LLM response)
_MITCHELL_IMAGES_STORE = {}
...
def process_tool_result(
    ...
    # Extract Mitchell static image URLs from tool result
    # Store in global dict for later injection into response (not as file attachments)
    # This keeps AI writing words, Python handling images
    if isinstance(tool_result, str) and "/static/mitchell/" in tool_result:
        import re
        import datetime
        mitchell_img_pattern = r'!\[([^\]]*)\]\((/static/mitchell/[^)]+)\)'
        mitchell_images = []
        for match in re.finditer(mitchell_img_pattern, tool_result):
            alt_text, img_url = match.groups()
            mitchell_images.append(f"![{alt_text}]({img_url})")
        
        if mitchell_images and metadata:
            # Store globally by message_id for injection after LLM response
            msg_id = metadata.get("message_id")
            if msg_id:
                _MITCHELL_IMAGES_STORE[msg_id] = mitchell_images
                with open("/tmp/middleware_debug.log", "a") as f:
                    f.write(f"{datetime.datetime.now().isoformat()} STORED IMAGES for {msg_id}: {mitchell_images}\n")
        
        # Remove image markdown from text - LLM doesn't need to see it
        tool_result = re.sub(mitchell_img_pattern, '', tool_result).strip()
        ...
        try:
            ...
            async def tool_call_handler(tool_call):
                ...
                if tool_result:
                    ...
                    # DEBUG: Log tool result for Mitchell - ENABLED
                    # To disable, set _MITCHELL_DEBUG = False
                    _MITCHELL_DEBUG = True
                    if _MITCHELL_DEBUG and ("mitchell" in tool_name.lower() or "wiring" in tool_function_name.lower()):
                        import datetime
                        with open("/tmp/middleware_debug.log", "a") as f:
                            f.write(f"{datetime.datetime.now().isoformat()} TOOL RESULT\n")
                            f.write(f"  tool_name: {tool_name}\n")
                            f.write(f"  tool_result type: {type(tool_result)}\n")
                            f.write(f"  tool_result length: {len(str(tool_result))}\n")
                            f.write(f"  Contains '![': {'![' in str(tool_result)}\n")
                            f.write(f"  First 1000 chars: {str(tool_result)[:1000]}\n\n")
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # assign a per-request UUID to make tracing easier
    ...
    if system_message:  # Chat Controls/User Settings
        try:
            # TEMP DEBUG: log messages BEFORE applying system prompt
            try:
                ...
                for m in messages[:3]:
                    ...
                    if isinstance(c, str):
                        ...
                        content_preview = (c[0].get('text') or '')[:120]
                    msgs_preview.append(f"{m.get('role')}:{content_preview}")
                user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
                log.info(f"pre_apply_system_prompt: req={request_id[:8]}, user={str(user_id_val)[:8] if user_id_val else 'unknown'}, messages_preview={msgs_preview}, messages_len={len(messages)}")
            except Exception as e:
                log.debug(f"pre_apply_system_prompt_error: {e}")

            form_data = apply_system_prompt_to_body(
                system_message.get("content"), form_data, metadata, user, replace=True
            )  # Required to handle system prompt variables
            # TEMP DEBUG: inspect messages immediately after apply_system_prompt_to_body
            try:
                user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
                sys_msg = get_system_message(form_data.get('messages', []))
                sys_preview = (sys_msg.get('content')[:300] if sys_msg else '<no-system-message>')
                log.info(f"post_apply_system_prompt: req={request_id[:8]}, user={str(user_id_val)[:8] if user_id_val else 'unknown'}, system_preview={sys_preview}, messages_len={len(form_data.get('messages', []))}")
            except Exception as e:
                log.debug(f"post_apply_system_prompt_error: {e}")
        except:
            pass
    else:
        # No system message present — inject deterministic greeting for user
        try:
            form_data = ensure_system_greeting(form_data, metadata, user, request_id=request_id)
        except Exception:
            pass

    # TEMP DEBUG: log immediately after applying system prompt to confirm presence
    try:
        user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
        messages = form_data.get('messages', [])
        sys_msg = get_system_message(messages)
        sys_preview = (sys_msg.get('content')[:300] if sys_msg else '<no-system-message>')
        has_system = bool(sys_msg)
        # Construct concise preview of first up to 3 messages (role + truncated content)
        msgs_preview = []
        for m in (messages or [])[:3]:
            ...
            if isinstance(c, str):
                ...
                content_preview = (c[0].get('text') or '')[:120]
            msgs_preview.append(f"{m.get('role')}:{content_preview}")
        log.info(f"post_apply_system_prompt: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, has_system={has_system}, preview={sys_preview}, messages_preview={msgs_preview}, messages_len={len(messages)}")
    except Exception as e:
        log.debug(f"post_apply_system_prompt_error: {e}")
        ...
        if chat and chat.folder_id:
            ...
            if folder and folder.data:
                if "system_prompt" in folder.data:
                    # TEMP DEBUG: log messages BEFORE applying folder system prompt
                    try:
                        messages = form_data.get('messages', []) or []
                        msgs_preview = []
                        for m in messages[:3]:
                            ...
                            msgs_preview.append(f"{m.get('role')}:{content_preview}")
                        user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
                        log.info(f"pre_apply_system_prompt_folder: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, messages_preview={msgs_preview}, messages_len={len(messages)}")
                    except Exception as e:
                        log.debug(f"pre_apply_system_prompt_folder_error: {e}")

                    form_data = apply_system_prompt_to_body(
                        folder.data["system_prompt"], form_data, metadata, user
                    )

                    # TEMP DEBUG: log immediately after applying folder system prompt
                    try:
                        user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
                        messages = form_data.get('messages', [])
                        sys_msg = get_system_message(messages)
                        ...
                        for m in (messages or [])[:3]:
                            ...
                            if isinstance(c, str):
                                ...
                                content_preview = (c[0].get('text') or '')[:120]
                            msgs_preview.append(f"{m.get('role')}:{content_preview}")
                        log.info(f"post_apply_system_prompt_folder: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, has_system={has_system}, preview={sys_preview}, messages_preview={msgs_preview}, messages_len={len(messages)}")
                    except Exception as e:
                        log.debug(f"post_apply_system_prompt_folder_error: {e}")
    ...
    # TEMP DEBUG: log system message and messages length after pipeline inlet
    try:
        user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
        sys_msg = get_system_message(form_data.get('messages', []))
        sys_preview = (sys_msg.get('content')[:300] if sys_msg else '<no-system-message>')
        log.info(f"debug_pipeline_inlet_after: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, system_preview={sys_preview}, messages_len={len(form_data.get('messages', []))}")
    except Exception as e:
        log.debug(f"debug_pipeline_inlet_after_error: {e}")
    ...
    # TEMP DEBUG: log system message and messages length after filter functions (inlet)
    try:
        user_id_val = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
        sys_msg = get_system_message(form_data.get('messages', []))
        sys_preview = (sys_msg.get('content')[:300] if sys_msg else '<no-system-message>')
        log.info(f"debug_filter_inlet_after: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, system_preview={sys_preview}, messages_len={len(form_data.get('messages', []))}")
    except Exception as e:
        log.debug(f"debug_filter_inlet_after_error: {e}")
    ...
    if len(sources) > 0:
        ...
        # DEBUG: Log context for Mitchell tool debugging - ENABLED
        # To disable, set _MITCHELL_DEBUG = False
        _MITCHELL_DEBUG = True
        if _MITCHELL_DEBUG and ("mitchell" in context_string.lower() or "/static/mitchell" in context_string.lower()):
            import datetime
            with open("/tmp/middleware_debug.log", "a") as f:
                f.write(f"{datetime.datetime.now().isoformat()} CONTEXT STRING\n")
                f.write(f"  context_string length: {len(context_string)}\n")
                f.write(f"  Contains '![': {'![' in context_string}\n")
                f.write(f"  Contains '/static/mitchell/': {'/static/mitchell/' in context_string}\n")
                f.write(f"  First 1500 chars: {context_string[:1500]}\n\n")
        
        if prompt is None:
            raise Exception("No user message found")

        if context_string != "":
            full_rag_content = rag_template(
    ...
    if event_emitter and event_caller:
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                # Inject Mitchell images at end of response (Python handles display)
                msg_id = metadata.get("message_id")
                mitchell_images = _MITCHELL_IMAGES_STORE.pop(msg_id, None) if msg_id else None
                
                if mitchell_images:
                    image_section = "\n\n**IMAGES:**\n\n"
                    for img_md in mitchell_images:
                        image_section += f"{img_md}\n\n"
                    
                    # Append to last text block
                    if content_blocks and content_blocks[-1].get("type") == "text":
                        content_blocks[-1]["content"] += image_section
                    else:
                        content_blocks.append({"type": "text", "content": image_section})
