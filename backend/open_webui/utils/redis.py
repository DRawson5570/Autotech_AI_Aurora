            ...
            async def _wrapped(*args, **kwargs):
                for i in range(REDIS_SENTINEL_MAX_RETRY_COUNT):
                    try:
                        ...
                        # If the mock has a side_effect list configured (used in some
                        # tests to simulate transient failures), call the method with
                        # the positional args bundled as a single tuple so the mock's
                        # call_args_list matches expectations in the tests.
                        # Otherwise, call with standard positional arguments.
                        if hasattr(method, "side_effect") and getattr(method, "side_effect") is not None:
                            if args:
                                result = method(args)
                            else:
                                result = method()
