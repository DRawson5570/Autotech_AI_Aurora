...
async def get_all_base_models(request: Request, user: UserModel = None):
    ...
    # Google models will be fetched below if enabled
    ...
    # If Google is enabled, fetch google models and include them
    google_models = []
    try:
        if request.app.state.config.ENABLE_GOOGLE_API:
            try:
                ...
                log.info(f"DEBUG models.py: calling google.get_all_models")
                result = google.get_all_models(request, user=user)
                log.info(f"DEBUG models.py: result type = {type(result)}")
                raw_google_models = await result or []
                log.info(f"DEBUG models.py: raw_google_models count = {len(raw_google_models)}")
                log.info(f"DEBUG models.py: raw_google_models type = {type(raw_google_models)}")
                if raw_google_models:
                    log.info(f"DEBUG models.py: first model = {raw_google_models[0] if raw_google_models else 'empty'}")
                # Check for gemini-3 models
                g3_models = [m for m in raw_google_models if 'gemini-3' in (m.get('id') or '')]
                log.info(f"DEBUG models.py: gemini-3 models in raw = {[m.get('id') for m in g3_models]}")
                ...
                google_models = [
                    ...
                    for m in (raw_google_models or [])
                    if m and m.get("id")
                ]
                log.info(f"DEBUG models.py: google_models count after processing = {len(google_models)}")

                # If we couldn't fetch any models (e.g., missing API key), provide a conservative
                # fallback list so the UI shows Google options and the admin can add credentials.
                if not google_models:
                    default_google_models = [
                        {"id": "models/gemini-2.5-flash", "name": "Gemini 2.5 Flash"},
                        {"id": "models/gemini-2.5-pro", "name": "Gemini 2.5 Pro"},
                        {"id": "models/gemini-2.5", "name": "Gemini 2.5"},
                        {"id": "models/gemini-2.5-flash-lite", "name": "Gemini 2.5 Flash Lite"},
                        {"id": "models/gemini-2.0-flash", "name": "Gemini 2.0 Flash"},
                        {"id": "models/gemini-2.0-flash-001", "name": "Gemini 2.0 Flash (v1)"},
                        {"id": "models/gemini-2.0-flash-lite-001", "name": "Gemini 2.0 Flash Lite (v1)"},
                        {"id": "models/gemini-2.0-flash-lite", "name": "Gemini 2.0 Flash Lite"},
                        {"id": "models/gemini-1.5-pro", "name": "Gemini 1.5 Pro"},
                        {"id": "models/embedding-001", "name": "Google Embedding 001"},
                        {"id": "models/text-embedding-004", "name": "Google Text Embedding 004"},
                    ]

                    for m in default_google_models:
                        mid = m["id"].split("/")[-1] if isinstance(m.get("id"), str) else m.get("id")
                        google_models.append(
                            {
                                ...
                                "google": {},
                                "connection_type": "external",
                                "tags": [{"name": "requires-google-credentials"}],
                                "info": {"meta": {"note": "Requires Google API credentials to use (configure in Admin → Connections)."}},
                            }
                        )

                # Populate a mapping for quick lookups by id, similar to OPENAI_MODELS
                try:
                    request.app.state.GOOGLE_MODELS = {g["id"]: g for g in google_models}
                except Exception:
                    request.app.state.GOOGLE_MODELS = {}


            except Exception as e:
                log.debug(f"Failed to fetch google models: {e}")
    except Exception:
        google_models = []

    return function_models + openai_models + ollama_models + google_models
