...
def apply_system_prompt_to_body(
    ...
    # Debug: log user info before template replacement
    if user and "{{USER_" in system:
        user_name = user.name if hasattr(user, 'name') else (user.get('name') if isinstance(user, dict) else None)
        user_id = user.id if hasattr(user, 'id') else (user.get('id') if isinstance(user, dict) else None)
        log.info(f"apply_system_prompt_to_body: user_id={str(user_id)[:8] if user_id else 'None'}, name='{user_name}'")
    
    # Legacy (API Usage)
    system = prompt_template(system, user)

    # Log a short preview and which user name was applied (low-noise; safe for INFO)
    try:
        import re

        m = re.search(r"Hello\s+([A-Z][a-zA-Z'\-]+)\b", system)
        greeted = m.group(1) if m else None
    except Exception:
        greeted = None

    # Avoid logging PII - log user id prefix and greeting preview only
    user_id = None
    if user:
        try:
            user_id = user.model_dump().get("id") if hasattr(user, "model_dump") else user.get("id")
        except Exception:
            try:
    ...
    if metadata and isinstance(metadata, dict):
        metadata_chat = metadata.get("chat_id")

    try:
        log.info(
            f"apply_system_prompt: user={str(user_id)[:8] if user_id else 'unknown'}, greeted={greeted}, model={form_data.get('model')}, chat={metadata_chat}"
        )
    except Exception:
        pass

    # TEMP DEBUG: capture messages BEFORE modification
    try:
        messages_before = form_data.get('messages', []) or []
        msgs_preview_before = []
        for m in messages_before[:3]:
            c = m.get('content')
            if isinstance(c, str):
                cp = c[:120]
            elif isinstance(c, list) and len(c) > 0:
                cp = (c[0].get('text') or '')[:120]
            else:
                cp = ''
            msgs_preview_before.append(f"{m.get('role')}:{cp}")
        log.info(f"apply_system_prompt_before: user={str(user_id)[:8] if user_id else 'unknown'}, messages_preview={msgs_preview_before}, messages_len={len(messages_before)}")
    except Exception:
        log.debug("apply_system_prompt_before_error")
    ...
    # TEMP DEBUG: capture messages AFTER modification
    try:
        from open_webui.utils.misc import get_system_message

        messages_after = form_data.get('messages', []) or []
        msgs_preview_after = []
        for m in messages_after[:3]:
            c = m.get('content')
            if isinstance(c, str):
                cp = c[:120]
            elif isinstance(c, list) and len(c) > 0:
                ...
                cp = ''
            msgs_preview_after.append(f"{m.get('role')}:{cp}")

        sys_msg = get_system_message(messages_after)
        sys_preview = (sys_msg.get("content")[:300] if sys_msg else '<no-system-message>')
        user_id_val = None
        if user:
            try:
                user_id_val = user.model_dump().get("id") if hasattr(user, "model_dump") else (user.get('id') if isinstance(user, dict) else getattr(user, 'id', None))
            except Exception:
                user_id_val = getattr(user, 'id', None)
        log.info(f"apply_system_prompt_after: user={str(user_id_val)[:8] if user_id_val else 'unknown'}, has_system={bool(sys_msg)}, preview={sys_preview}, messages_preview={msgs_preview_after}, messages_len={len(messages_after)}")
    except Exception:
        log.debug("apply_system_prompt_after_error")
