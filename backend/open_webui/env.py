...
# Optional friendly name to show to regular users (e.g., a tenant or instance display name).
# It will be normalized to include the "(Open WebUI)" suffix when not equal to the default.
USER_WEBUI_NAME = os.environ.get("USER_WEBUI_NAME", "")
if USER_WEBUI_NAME:
    if USER_WEBUI_NAME != "Open WebUI":
        USER_WEBUI_NAME += " (Open WebUI)"
else:
    USER_WEBUI_NAME = None

# Optional JSON mapping of model_id -> display name to show for normal users
# Example: export MODEL_ALIASES='{"llama3.1:8b":"Aurora AI"}'
import json
MODEL_ALIASES = None
_raw_model_aliases = os.environ.get("MODEL_ALIASES", "")
if _raw_model_aliases:
    try:
        MODEL_ALIASES = json.loads(_raw_model_aliases)
    except Exception:
        log = logging.getLogger(__name__)
        log.exception("Failed to parse MODEL_ALIASES env var as JSON: %s", _raw_model_aliases)
        MODEL_ALIASES = None
...
ENABLE_TRAINING_DATA_COLLECTION = (
    os.environ.get("ENABLE_TRAINING_DATA_COLLECTION", "True").lower() == "true"
)
...
# Google API retry settings
GOOGLE_API_MAX_RETRIES = int(os.environ.get("GOOGLE_API_MAX_RETRIES", "6"))
GOOGLE_API_RETRY_DELAY = float(os.environ.get("GOOGLE_API_RETRY_DELAY", "2.0"))
GOOGLE_API_FAILOVER_TIMEOUT = float(os.environ.get("GOOGLE_API_FAILOVER_TIMEOUT", "30.0"))  # seconds

# Google model failover mappings
# Configure via env vars: GOOGLE_FAILOVER_<MODEL>=<FALLBACK_MODEL>
# Example: GOOGLE_FAILOVER_gemini-3-flash-preview=gemini-2.5-flash
# Model names with dashes should use dashes in env var names
GOOGLE_MODEL_FAILOVER: dict[str, str] = {}
for key, value in os.environ.items():
    if key.startswith("GOOGLE_FAILOVER_"):
        model_name = key[len("GOOGLE_FAILOVER_"):].replace("_", "-").lower()
        # Also store with underscores in case that's how it's referenced
        GOOGLE_MODEL_FAILOVER[model_name] = value
        # Handle both dash and underscore versions
        GOOGLE_MODEL_FAILOVER[model_name.replace("-", "_")] = value
        log.debug(f"Registered Google model failover: {model_name} -> {value}")

# Billing: token bundle options for purchase (comma-separated list of integers)
_token_bundles_raw = os.environ.get("BILLING_TOKEN_BUNDLES", "10000,20000,50000")
BILLING_TOKEN_BUNDLES = [int(x.strip()) for x in _token_bundles_raw.split(",") if x.strip()]

# Billing: Subscription tier definitions
# Configure via env vars: BILLING_TIER_<ID>=name:monthly_price_cents:tokens_included:overage_rate_per_1k_cents
# Example: BILLING_TIER_STARTER=Starter:2000:400000:5 means $20/mo, 400K tokens, $0.05 per 1K overage

def _parse_tier_env(tier_id: str, env_value: str) -> dict:
    """Parse tier env var format: name:price_cents:tokens:overage_rate"""
    parts = env_value.split(":")
    if len(parts) != 4:
        raise ValueError(f"Invalid tier format for {tier_id}: expected name:price:tokens:overage")
    return {
        "name": parts[0],
        "monthly_price_cents": int(parts[1]),
        "tokens_included": int(parts[2]),
        "overage_rate_per_1k_cents": int(parts[3]),
    }

# Default tier configs (can be overridden via BILLING_TIER_* env vars)
_DEFAULT_TIERS = {
    "starter": "Starter:2000:400000:5",      # $20/mo, 400K tokens, $0.05/1K overage
    "pro": "Pro:3000:800000:4",               # $30/mo, 800K tokens, $0.04/1K overage
    "enterprise": "Enterprise:4000:1500000:3", # $40/mo, 1.5M tokens, $0.03/1K overage
}

BILLING_TIERS = {
    # "none" tier is always present for cancelled/no-plan users
    "none": {
        "name": "No Plan",
...
# Load tiers from env vars or use defaults
for tier_id, default_value in _DEFAULT_TIERS.items():
    env_value = os.environ.get(f"BILLING_TIER_{tier_id.upper()}", default_value)
    BILLING_TIERS[tier_id] = _parse_tier_env(tier_id, env_value)

# Default tier for new users (set to None for pay-per-token model)
BILLING_DEFAULT_TIER = os.environ.get("BILLING_DEFAULT_TIER", None)
