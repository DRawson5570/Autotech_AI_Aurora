...
def get_event_emitter(request_info, update_db=True):
    async def __event_emitter__(event_data):
        ...
        # Inject model_display into event payloads when possible so clients get aliases immediately
        try:
            from open_webui.models.users import Users
            from open_webui.models.chats import Chats
            from open_webui.main import app as _app

            # If event contains a chat/message reference, try to enrich the payload from DB
            if isinstance(event_data, dict) and event_data.get("type") in ("message", "replace", "chat:message"):
                # Attempt to retrieve latest message state from DB
                try:
                    if message_id:
                        db_message = Chats.get_message_by_id_and_message_id(chat_id, message_id)
                    else:
                        db_message = None
                except Exception:
                    db_message = None

                # If a DB message exists and has a model (or models), and an alias is configured, add model_display
                if db_message and isinstance(db_message, dict):
                    aliases = getattr(_app.state, "MODEL_ALIASES", None) or {}

                    # If aliases empty, try to parse from environment (runtime fallback)
                    if not aliases:
                        try:
                            import os, json

                            raw = os.environ.get("MODEL_ALIASES", "") or ""
                            if raw:
                                aliases = json.loads(raw)
                                try:
                                    _app.state.MODEL_ALIASES = aliases
                                except Exception:
                                    pass
                        except Exception:
                            aliases = {}

                    # Collect possible model ids from DB message
                    model_ids = []
                    if db_message.get("model"):
                        model_ids.append(db_message.get("model"))
                    if db_message.get("selectedModelId"):
                        model_ids.append(db_message.get("selectedModelId"))
                    if db_message.get("modelName"):
                        model_ids.append(db_message.get("modelName"))
                    if isinstance(db_message.get("models"), list):
                        model_ids.extend([x for x in db_message.get("models") if isinstance(x, str)])

                    alias_to_use = None
                    for mid in model_ids:
                        if aliases and aliases.get(mid):
                            alias_to_use = aliases.get(mid)
                            break

                    if alias_to_use:
                        # Check user role; only non-admins get alias
                        try:
                            usr = Users.get_user_by_id(user_id)
                            if not (usr and getattr(usr, "role", None) == "admin"):
                                # Inject model_display into the payload data.message (if present) or set so clients can read it
                                if isinstance(event_data.get("data"), dict):
                                    if "message" in event_data["data"] and isinstance(event_data["data"]["message"], dict):
                                        event_data["data"]["message"]["model_display"] = alias_to_use
                                    else:
                                        # Add a message object fragment with model_display for clients that expect it
                                        event_data["data"]["model_display"] = alias_to_use

                                # Persist alias into DB so subsequent GETs include model_display
                                try:
                                    if update_db and message_id and not request_info.get("chat_id", "").startswith("local:"):
                                        Chats.upsert_message_to_chat_by_id_and_message_id(
                                            request_info["chat_id"], request_info["message_id"], {"model_display": alias_to_use}
                                        )
                                except Exception:
                                    pass
                        except Exception:
            ...
            # Non-fatal: do not prevent emitting events if enrichment fails
            pass
        ...
        if (
            ...
            if "type" in event_data and event_data["type"] == "message":
                ...
                if message:
                    ...
                    # If the payload includes token usage information, record it and persist to the message
                    try:
                        usage = event_data.get("data", {}).get("usage") or event_data.get("data", {}).get("tokens")
                        if usage and isinstance(usage, dict):
                            # Standard shapes: {prompt_tokens, completion_tokens, total_tokens}
                            ...
                            total_tokens = usage.get("total_tokens") or usage.get("total") or int(prompt_tokens or 0) + int(completion_tokens or 0)

                            # Persist token fields into message JSON for later visibility
                            Chats.upsert_message_to_chat_by_id_and_message_id(
                                request_info["chat_id"],
                                request_info["message_id"],
                                {
                            ...
                            # Record usage event and update user aggregate
                            try:
                                from open_webui.models.billing import record_usage_event, check_and_trigger_auto_renew
                                # Determine user id from request_info
                                uid = request_info.get("user_id")
                                record_usage_event(
                                    uid,
                                    ...
                                    token_source=usage.get("source") if isinstance(usage.get("source"), str) else None,
                                )
                                # Check for auto-renew (async, non-blocking)
                                if uid:
                                    try:
                                        check_and_trigger_auto_renew(uid)
                                    except Exception:
                                        pass
                            except Exception:
                                # Non-fatal: don't block emitter
                                pass
                        else:
                            # No provider usage supplied: compute tokens from the content and record
                            try:
                                if content and isinstance(content, str) and content.strip():
                                    compute_tokens_and_record_message(request_info.get("user_id"), request_info["chat_id"], request_info["message_id"], content, request_info.get("model"))
                            except Exception:
                                pass
                    except Exception:
                        pass
...
def compute_tokens_and_record_message(user_id: str, chat_id: str, message_id: str, content: str, model_name: str | None = None):
    """Compute tokens (fallback) and persist + record usage for a message.

    Top-level helper used when provider does not supply token usage info.
    """
    try:
        if not content or not isinstance(content, str) or not content.strip():
            log.debug(f"No content to compute tokens for message {message_id} in chat {chat_id}")
            return
        ...
        tokens_total = int(count_tokens_for_model(content or "", model_name))
        tokens_completion = int(tokens_total)
        tokens_prompt = 0
        ...
        # Record and log for observability
        record_usage_event(user_id, chat_id, message_id, tokens_prompt, tokens_completion, tokens_total, token_source="computed")
        log.info(f"Computed tokens (total={tokens_total}) for user {user_id} chat {chat_id} message {message_id} (source=computed)")
        
        # Check for auto-renew (non-blocking)
        if user_id:
            try:
                check_and_trigger_auto_renew(user_id)
            except Exception:
                pass
    except Exception as e:
        # Non-fatal: don't block emitter
        log.warning(f"Error computing tokens for message {message_id}: {e}")
        pass
