...
def generate_ollama_batch_embeddings(
    ...
    """
    Generate embeddings using Ollama with timeout and retry logic (sync version).
    
    Args:
        timeout_seconds: Timeout per request (default 10 min for large batches)
    ...
    for attempt in range(max_retries):
        try:
            log.info(
                f"generate_ollama_batch_embeddings: model={model} batch_size={len(texts)} "
            ...
            if "embeddings" in data:
                log.info(
                    f"generate_ollama_batch_embeddings: SUCCESS - got {len(data['embeddings'])} embeddings"
                )
            ...
            last_error = e
            log.warning(
                f"generate_ollama_batch_embeddings: TIMEOUT after {timeout_seconds}s "
            ...
            last_error = e
            log.warning(
                f"generate_ollama_batch_embeddings: ERROR {type(e).__name__}: {e} "
                f"(attempt {attempt + 1}/{max_retries})"
            )
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)
    ...
    log.error(
        f"generate_ollama_batch_embeddings: FAILED after {max_retries} attempts. "
...
async def agenerate_ollama_batch_embeddings(
    ...
    for attempt in range(max_retries):
        try:
            log.info(
                f"agenerate_ollama_batch_embeddings: model={model} batch_size={len(texts)} "
                f"attempt={attempt + 1}/{max_retries} timeout={timeout_seconds}s"
            )
            ...
            async with aiohttp.ClientSession(trust_env=True, timeout=timeout) as session:
                async with session.post(
                    ...
                    if "embeddings" in data:
                        log.info(
                            f"agenerate_ollama_batch_embeddings: SUCCESS - got {len(data['embeddings'])} embeddings"
                        )
                        return data["embeddings"]
                    else:
                        raise Exception(f"Unexpected response format: {list(data.keys())}")
            ...
            last_error = e
            log.warning(
                f"agenerate_ollama_batch_embeddings: TIMEOUT after {timeout_seconds}s "
                f"(attempt {attempt + 1}/{max_retries})"
            )
            if attempt < max_retries - 1:
            ...
            last_error = e
            log.warning(
                f"agenerate_ollama_batch_embeddings: ERROR {type(e).__name__}: {e} "
                f"(attempt {attempt + 1}/{max_retries})"
            )
            if attempt < max_retries - 1:
    ...
    log.error(
        f"agenerate_ollama_batch_embeddings: FAILED after {max_retries} attempts. "
        f"Last error: {last_error}"
    )
    return None
    ...
    if embedding_engine == "":
        ...
        async def async_embedding_function(query, prefix=None, user=None):
            if isinstance(query, list):
                ...
                total_texts = len(query)
                log.info(
                    f"[EMBEDDING] Starting: {total_texts} texts in {len(batches)} batches "
                    f"(batch_size={embedding_batch_size}, engine={embedding_engine})"
                )

                if enable_async:
                    log.info(f"[EMBEDDING] Processing {len(batches)} batches in parallel")
                    ...
                    log.info(f"[EMBEDDING] Processing {len(batches)} batches sequentially")
                    batch_results = []
                    for batch_idx, batch in enumerate(batches):
                        log.info(f"[EMBEDDING] Batch {batch_idx + 1}/{len(batches)} ({len(batch)} texts)...")
                        batch_results.append(
                            await embedding_function(batch, prefix=prefix, user=user)
                        )

                # Flatten results, checking for failures
                embeddings = []
                for batch_idx, batch_embeddings in enumerate(batch_results):
                    if batch_embeddings is None:
                        raise ValueError(
                            f"Embedding batch {batch_idx + 1}/{len(batches)} failed - "
                            f"embedding service returned None. Check embedding service logs."
                        )
                    if isinstance(batch_embeddings, list):
                        embeddings.extend(batch_embeddings)
                    else:
                        raise ValueError(
                            f"Embedding batch {batch_idx + 1}/{len(batches)} returned unexpected type: "
                            f"{type(batch_embeddings)}"
                        )

                log.info(
                    f"[EMBEDDING] Complete: {len(embeddings)} embeddings from {len(batches)} batches"
                )
                return embeddings
            else:
