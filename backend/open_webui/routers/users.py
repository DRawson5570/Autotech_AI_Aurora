...
async def update_user_by_id(
    ...
    # Check if this is a pending → user transition (for welcome email)
    user = Users.get_user_by_id(user_id)
    was_pending = user and user.role == "pending"
    becoming_user = form_data.role == "user"
    ...
    if user:
        ...
        if updated_user:
            # Send welcome email if user was just approved (pending → user)
            if was_pending and becoming_user:
                try:
                    from open_webui.utils.email import send_welcome_email
                    send_welcome_email(updated_user.name, updated_user.email)
                    log.info(f"Welcome email sent to {updated_user.email}")
                except Exception as e:
                    log.warning(f"Failed to send welcome email to {updated_user.email}: {e}")
