...
try:
    ...
    # ldap3 is optional for local/dev environments — provide safe fallbacks
    Server = Connection = NONE = Tls = None
...
async def get_session_user(
    ...
    # Include group memberships (id, name, data) so frontend can enforce group-level settings
    groups = Groups.get_groups_by_member_id(user.id)

    # Expose the current token pricing so the frontend can display accurate bundle prices.
    try:
        token_usd_rate = float(getattr(request.app.state.config, "BILLING_TOKEN_USD_RATE"))
    except Exception:
        token_usd_rate = float(os.environ.get("BILLING_TOKEN_USD_RATE", "0.00002"))
    ...
    return {
        ...
        "groups": [{"id": g.id, "name": g.name, "data": g.data} for g in groups],
    }
...
async def signin(request: Request, response: Response, form_data: SigninForm):
    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        # Normalize email and ensure password length is safe for bcrypt
        email_normalized = form_data.email.lower()
        ...
        # Direct authentication: lookup user by email, then auth record by id and verify password
        try:
            user = Users.get_user_by_email(email_normalized)
            if user:
                from open_webui.internal.db import get_db
                from open_webui.models.auths import Auth as AuthDB

                with get_db() as db:
                    auth_rec = db.query(AuthDB).filter_by(id=user.id, active=True).first()
                    if auth_rec:
                        if verify_password(form_data.password, auth_rec.password):
                            log.info(f"signin: password verified for user={email_normalized}")
                        else:
                            log.info(f"signin: password verification failed for user={email_normalized}")
                            user = None
                    else:
                        log.info(f"signin: no auth record for user id={user.id}")
                        user = None
            else:
                log.info(f"signin: no user record for email={email_normalized}")
        except Exception as e:
            log.exception(f"signin: unexpected error during auth lookup: {e}")
            user = None
