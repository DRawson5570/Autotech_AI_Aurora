...
class OllamaConfigForm(BaseModel):
    ...
    OLLAMA_BASE_URLS: Optional[list[str]] = None
    OLLAMA_API_CONFIGS: Optional[dict] = None
...
async def update_config(
    ...
    """Update Ollama config; only modify fields that are provided in the payload."""

    if form_data.ENABLE_OLLAMA_API is not None:
        request.app.state.config.ENABLE_OLLAMA_API = form_data.ENABLE_OLLAMA_API

    if form_data.OLLAMA_BASE_URLS is not None:
        request.app.state.config.OLLAMA_BASE_URLS = form_data.OLLAMA_BASE_URLS

    if form_data.OLLAMA_API_CONFIGS is not None:
        request.app.state.config.OLLAMA_API_CONFIGS = form_data.OLLAMA_API_CONFIGS

        # Remove the API configs that are not in the API URLS
        keys = list(map(str, range(len(request.app.state.config.OLLAMA_BASE_URLS or []))))
