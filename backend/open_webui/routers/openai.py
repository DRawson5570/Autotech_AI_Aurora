...
class OpenAIConfigForm(BaseModel):
    ...
    OPENAI_API_BASE_URLS: Optional[list[str]] = None
...
async def update_config(
    ...
    """Update OpenAI config; only modify fields that are provided in the request payload."""

    # Enable flag
    if form_data.ENABLE_OPENAI_API is not None:
        request.app.state.config.ENABLE_OPENAI_API = form_data.ENABLE_OPENAI_API

    # Update base URLs / keys only when provided
    if form_data.OPENAI_API_BASE_URLS is not None:
        base_urls = form_data.OPENAI_API_BASE_URLS or []
        keys = form_data.OPENAI_API_KEYS or []

        request.app.state.config.OPENAI_API_BASE_URLS = base_urls

        # Ensure keys length matches urls
    ...
    # Update configs only when provided
    if form_data.OPENAI_API_CONFIGS is not None:
        request.app.state.config.OPENAI_API_CONFIGS = form_data.OPENAI_API_CONFIGS

        # Remove the API configs that are not in the API URLS (if urls exist)
        keys_range = list(map(str, range(len(request.app.state.config.OPENAI_API_BASE_URLS or []))))
        request.app.state.config.OPENAI_API_CONFIGS = {
            key: value
            for key, value in request.app.state.config.OPENAI_API_CONFIGS.items()
            if key in keys_range
        }
            ...
            try:
                ...
                if api_config.get("azure", False):
                    ...
                    async with session.get(
                        ...
                        if r.status != 200:
                            # Propagate upstream failures (e.g. missing API key) instead of converting them into 500s.
                            # Keep the original error shape when possible.
                            if isinstance(response_data, (dict, list)):
                                return JSONResponse(status_code=r.status, content=response_data)
                            return JSONResponse(
                                status_code=r.status,
                                content={"error": {"message": str(response_data)}},
                            )

                        # Check if we're calling OpenAI API based on the URL
                        if isinstance(response_data, dict) and "api.openai.com" in url:
