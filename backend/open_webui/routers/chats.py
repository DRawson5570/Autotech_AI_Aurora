...
async def get_chat_by_id(id: str, request: Request, user=Depends(get_verified_user)):
    ...
    if chat:
        _apply_model_aliases_to_chat(chat, request, user)
        return ChatResponse(**chat.model_dump())
...
async def update_chat_by_id(
    id: str, form_data: ChatForm, request: Request, user=Depends(get_verified_user)
):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
...
def _apply_model_aliases_to_chat(chat_obj, request: Request, user):
    """If MODEL_ALIASES are configured and the current user is not admin,
    add a `model_display` key to assistant messages with an alias when available.
    Modifies chat_obj.chat in-place. This supports multiple message shapes
    (single `model`, `selectedModelId`, `modelName`, or a `models` list) and
    will attempt to parse `MODEL_ALIASES` from the environment if it wasn't
    populated into `app.state` at startup.
    """
    try:
        aliases = getattr(request.app.state, "MODEL_ALIASES", None) or {}
    except Exception:
        aliases = {}

    # If aliases are empty, attempt to parse them from the env var at runtime
    if not aliases:
        try:
            import os, json
            ...
            if raw:
                ...
                try:
                    request.app.state.MODEL_ALIASES = aliases
                except Exception:
                    pass
        except Exception:
            aliases = {}
    ...
    chat_data = chat_obj.chat or {}

    # Support multiple chat schemas: top-level 'messages' or nested under 'history' -> 'messages'
    messages = None
    if chat_data:
        if isinstance(chat_data.get("messages"), (dict, list)):
            messages = chat_data.get("messages")
        elif isinstance(chat_data.get("history", {}).get("messages"), (dict, list)):
            messages = chat_data["history"]["messages"]

    def _extract_model_ids(m: dict) -> list:
        """Return a list of possible model ids from a message dict."""
        ids = []
        if not isinstance(m, dict):
            return ids
        if m.get("model"):
            ids.append(m.get("model"))
        if m.get("selectedModelId"):
            ids.append(m.get("selectedModelId"))
        if m.get("modelName"):
            ids.append(m.get("modelName"))
        if isinstance(m.get("models"), list):
            ids.extend([x for x in m.get("models") if isinstance(x, str)])
        return ids

    # Apply the first matching alias found for each message
    if isinstance(messages, dict):
        for mid, m in messages.items():
            if not isinstance(m, dict):
                continue
            for mdl in _extract_model_ids(m):
                alias = aliases.get(mdl)
                if alias:
                    m["model_display"] = alias
                    try:
                        log.debug("Applied model_display='%s' to chat %s message %s for user %s", alias, getattr(chat_obj, 'id', None), mid, getattr(user, 'id', None))
                    except Exception:
                        pass
                    break
        ...
        for m in messages:
            ...
            for mdl in _extract_model_ids(m):
                ...
                if alias:
                    m["model_display"] = alias
                    try:
                        log.debug("Applied model_display='%s' to chat %s (list message) for user %s", alias, getattr(chat_obj, 'id', None), getattr(user, 'id', None))
                    except Exception:
                        pass
                    break

    # Ensure chat_obj.chat is updated
