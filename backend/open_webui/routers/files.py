...
def process_uploaded_file(request, file, file_path, file_item, file_metadata, user):
    """
    Process an uploaded file (extract text, process audio, etc).
    
    This function is designed to be resilient - it will catch ALL exceptions,
    log them, and mark the file as failed rather than crashing.
    """
    try:
        log.info(f"[PROCESS_UPLOAD] Starting: {file_item.filename} (id={file_item.id}, type={file.content_type})")
        ...
        log.info(f"[PROCESS_UPLOAD] Completed: {file_item.filename}")

    except Exception as e:
        # Catch ALL exceptions - we want to mark the file as failed, not crash
        error_msg = str(e.detail) if hasattr(e, "detail") else str(e)
        log.error(f"[PROCESS_UPLOAD] FAILED: {file_item.filename} (id={file_item.id})")
        log.error(f"[PROCESS_UPLOAD] Error: {error_msg}")
        ...
        try:
            ...
            log.error(f"[PROCESS_UPLOAD] Failed to update file status in DB: {db_error}")
...
async def update_file_data_content_by_id(
    ...
    if (
        ...
        try:
            ...
            # Re-index in all knowledge collections that contain this file
            linked_knowledges = Knowledges.get_knowledges_by_file_id(id)
            for knowledge in linked_knowledges:
                try:
                    # Delete old embeddings from knowledge collection
                    ...
                    log.info(f"Re-indexed file {id} in knowledge collection {knowledge.id}")
                except Exception as e:
                    log.error(f"Error re-indexing file {id} in knowledge {knowledge.id}: {e}")
