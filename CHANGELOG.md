...
- 💳 **Billing**: Prepaid token purchases with Stripe Checkout, webhook confirmation, and admin reconciliation flow (new `TokenPurchase` and `UserTokenBalance` models, reconcile scheduler and admin endpoints/ui).
- 🧾 **Admin UI**: "Reconcile All" button in Billing admin page with results table and polling refresh; frontend Vitest test added to cover bulk-reconcile flow.
- ✅ **Backend**: `/admin/purchases/reconcile_all` endpoint and unit test verifying reconcile helper invocation.
- 📊 **Observability**: Billing metrics helpers added with safe no-op behavior when metrics exporter not enabled (tested).
- 🔋 **Predictive Diagnostics**: Added EV and PHEV failure mode modules (`failures_ev.py`, `failures_phev.py`) and registered their systems/components in `FAILURE_REGISTRY` (systems added: `ev_battery`, `ev_powertrain`, `ev_charging`, `ev_thermal`, `phev_hybrid`, `phev_battery`, `phev_thermal`, `phev_powertrain`). Also added `FailureCategory.THERMAL` to support battery thermal events and unit tests to validate registration and DTC mappings.
- ⚡ **Predictive Diagnostics**: Added Tesla-specific failure modes (`failures_tesla.py`) and registered Tesla systems/components in `FAILURE_REGISTRY` (`tesla_hv_battery`, `tesla_powertrain`, `tesla_thermal`, `tesla_auxiliary`, `tesla_charging`). Added unit tests to validate Tesla registrations.

### Changed

- 🔧 **Tests**: Added frontend and backend tests for reconcile flows; full test suite verified locally (frontend + backend passing).
