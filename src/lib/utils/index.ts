...
export const processResponseContent = (content: string) => {
	...
	// Fix bare image paths from Mitchell tool that LLM sometimes fails to format as markdown
	// Converts: /static/mitchell/diagram_xxx.png -> ![Diagram](/static/mitchell/diagram_xxx.png)
	// Only converts if NOT already in markdown image syntax (![...](...) or [...](...))
	content = content.replace(
		/(?:^|[^(\]])\/static\/mitchell\/(diagram_[a-f0-9]+\.png)/gim,
		(match, filename) => {
			// Check if this is already part of markdown image/link syntax
			const prefix = match.charAt(0);
			if (prefix === '(' || prefix === ']') {
				return match; // Already in markdown syntax
			}
			// Preserve any leading character (newline, space, etc.) and add markdown image
			const leadingChar = match.startsWith('/static') ? '' : prefix;
			return `${leadingChar}![Diagram](/static/mitchell/${filename})`;
		}
	);
	return content.trim();
};
