<script lang="ts">
	...
	import { getMonthlyBill, getOverageStatus, type MonthlyBill, type OverageStatus } from '$lib/apis/billing';
	...
	let tokenBalanceLoading = false;
	let tokenBalanceFetchedForUserId: string | null = null;
	let monthlyBill: MonthlyBill | null = null;
	let lastShowSettings = false;
	let cleanupBalanceListeners: (() => void) | null = null;
	let lastOverageToastTime: number = 0;

	const checkAndShowOverageToast = async () => {
		const authToken = localStorage?.token ?? $user?.token;
		if (!authToken) return;
		
		// Only show toast once per 5 minutes to avoid spam
		const now = Date.now();
		if (now - lastOverageToastTime < 5 * 60 * 1000) return;
		
		try {
			const status = await getOverageStatus(authToken);
			if (status.in_overage && status.upgrade_tier && status.savings && status.savings > 0) {
				lastOverageToastTime = now;
				toast.warning(
					$i18n.t('You\'ve exceeded your {tier} plan tokens. Upgrade to {upgradeTier} to save ${savings}/mo!', {
						tier: status.current_tier?.charAt(0).toUpperCase() + status.current_tier?.slice(1),
						upgradeTier: status.upgrade_tier_name,
						savings: status.savings?.toFixed(2)
					}),
					{
						duration: 10000,
						action: {
							...
							onClick: () => {
								...
								settingsTab.set('account');
							}
						}
					}
				);
			} else if (status.in_overage && !status.upgrade_tier) {
				// In overage but no cheaper upgrade available
				lastOverageToastTime = now;
				toast.info(
					$i18n.t('You\'re using overage tokens. Estimated extra cost: ${cost}', {
						cost: status.projected_overage_cost?.toFixed(2)
					}),
					{ duration: 8000 }
				);
			}
	...
	const refreshTokenBalance = async () => {
		...
		try {
			const [bal, bill] = await Promise.all([
				getUserBalance(authToken),
				getMonthlyBill(authToken).catch(() => null)
			]);
			const rawBalance = bal?.tokens_balance;
			const nextBalance = rawBalance === null || rawBalance === undefined ? null : Number(rawBalance);
			tokenBalance = nextBalance !== null && Number.isFinite(nextBalance) ? nextBalance : null;
			monthlyBill = bill;
			
			// Check for overage and show toast if applicable
			if (tokenBalance !== null && tokenBalance < 0) {
				checkAndShowOverageToast();
			}
		} catch {
			tokenBalance = null;
			monthlyBill = null;
	...
	onMount(() => {
		const onBalanceUpdated = (event?: CustomEvent) => {
			const rawBalance = (event as any)?.detail?.tokens_balance;
			if (rawBalance !== null && rawBalance !== undefined) {
				const nextBalance = Number(rawBalance);
				tokenBalance = Number.isFinite(nextBalance) ? nextBalance : tokenBalance;
				return;
			}
			refreshTokenBalance();
		};
		...
		window.addEventListener('billing:balance-updated', onBalanceUpdated as any);
		document.addEventListener('visibilitychange', onVisibilityChange);

		cleanupBalanceListeners = () => {
			window.removeEventListener('billing:balance-updated', onBalanceUpdated as any);
	...
	$: if ($user?.id && tokenBalanceFetchedForUserId !== $user.id) {
		tokenBalanceFetchedForUserId = $user.id;
		tokenBalance = null;
		refreshTokenBalance();
	}

	$: if (!$user?.id) {
		tokenBalanceFetchedForUserId = null;
		tokenBalance = null;
	}

	$: {
		if (lastShowSettings && !$showSettings) {
			refreshTokenBalance();
		}
		...
		>
			...
			>
				<div class="pb-1.5">
					...
					{#if $user?.id}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							<div class="grow flex flex-col rounded-2xl px-2.5 py-1">
								<div class="self-start text-xs text-gray-500 dark:text-gray-400">
									{$i18n.t('Token balance')}: {tokenBalanceLoading ? '…' : tokenBalance ?? '—'}
								</div>
								{#if monthlyBill && monthlyBill.usage_percent >= 80}
									<div class="mt-1 flex items-center gap-1.5">
										{#if monthlyBill.usage_percent >= 100}
											<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">
												🚨 {monthlyBill.usage_percent.toFixed(0)}% — Overage
											</span>
										{:else}
											<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
												⚠️ {monthlyBill.usage_percent.toFixed(0)}% used
											</span>
										{/if}
									</div>
								{/if}
						...
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							<button
								id="sidebar-buy-tokens-button"
								class="grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 hover:bg-gray-100 dark:hover:bg-gray-900 transition text-left"
								on:click={(e) => {
									e.stopPropagation();
									settingsTab.set('account');
									showSettings.set(true);
								...
								aria-label={$i18n.t('Token Dashboard')}
							>
								<div class="self-center text-teal-500 dark:text-teal-400">
									<Sparkles className="size-4.5" strokeWidth="2" />
								</div>
								<div class="flex flex-1 self-center translate-y-[0.5px] text-teal-500 dark:text-teal-400">
									<div class="self-center text-sm font-primary">{$i18n.t('Token Dashboard')}</div>
								</div>
							</button>
						</div>
					{/if}
