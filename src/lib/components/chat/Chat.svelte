<script lang="ts">
	...
	import { scannerConnected, scannerVIN, scannerDTCs, scannerPIDs } from '$lib/stores/scanner';
	...
	$: customBgUrl =
		($settings?.backgroundImageUrl?.trim() ?? '') !== ''
			...
			: ($config?.license_metadata?.background_image_url?.trim() ?? '') !== ''
	...
	const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
		...
		// Build scanner context if connected
		let scannerContext = '';
		if ($scannerConnected) {
			const dtcList = $scannerDTCs.map(d => `${d.code} (${d.status})`).join(', ') || 'None';
			const pidEntries = Object.entries($scannerPIDs)
				.map(([k, v]) => `${k}: ${typeof v === 'number' ? v.toFixed(2) : v}`)
				.join(', ') || 'None read';
			
			scannerContext = `\n\n[OBD SCANNER DATA - LIVE FROM VEHICLE]
VIN: ${$scannerVIN || 'Not available'}
DTCs: ${dtcList}
Live PIDs: ${pidEntries}
Note: This data was read directly from the vehicle's OBD port. Use it to diagnose the vehicle.`;
		}

		let messages = [
			params?.system || $settings.system || scannerContext
				? {
						role: 'system',
						content: `${params?.system ?? $settings?.system ?? ''}${scannerContext}`
					}
				: undefined,
	...
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			...
			{:else if customBgUrl}
				...
				/>
			{:else}
				<!-- Aurora background image for automotive theme -->
				<div
					class="absolute top-0 left-0 w-full h-full bg-cover bg-center bg-no-repeat"
					style="background-image: url('/static/aurora.jpg')"
				/>

				<div
					class="absolute top-0 left-0 w-full h-full bg-linear-to-t from-white/70 to-white/40 dark:from-gray-900/80 dark:to-gray-900/60 z-0"
				/>
			{/if}
