<script lang="ts">
	...
	// Determine a canonical message model id from multiple potential fields
	$: messageModelId =
		message.model ??
		message.modelName ??
		(Array.isArray(message.models) && message.models.length ? message.models[0] : undefined) ??
		message.selectedModelId ??
		message.model;
	$: model = $models.find((m) => m.id === messageModelId);
	// If backend provides model_aliases, and the current session user is a normal 'user', prefer the alias for display when available
	$: if ($config?.model_aliases && ($user?.role ?? 'user') !== 'admin') {
		// Show alias for any non-admin role (or when not logged in); check canonical model id
		modelAlias = $config.model_aliases[messageModelId] ?? null;
	} else {
		modelAlias = null;
	}

	// Prefer server-provided model display (message.model_display) if present; fall back to client-side alias then model name.
	// For standard users, show instance branding instead of the raw model name.
	$: displayModelName = ($user?.role ?? 'user') === 'admin'
		? (message?.model_display ? message.model_display : modelAlias ?? model?.name ?? messageModelId)
		: 'AutoTech AI';

	// Split displayModelName into prefix + suffix when the name ends with 'AI' so we can color the 'AI' suffix to match branding
	let displayModelPrefix = '';
	let displayModelSuffix = '';
	$: {
		if (displayModelName) {
			const m = displayModelName.match(/^(.*?)(\s*AI\b)$/i);
			if (m) {
				displayModelPrefix = m[1];
				displayModelSuffix = m[2];
			} else {
				displayModelPrefix = displayModelName;
				displayModelSuffix = '';
			}
		} else {
			displayModelPrefix = '';
...
<Tooltip content={displayModelName} placement="top-start">
				...
				{#if displayModelSuffix}
					{displayModelPrefix}<span class="text-teal-400">{displayModelSuffix}</span>
				{:else}
					{displayModelName}
				{/if}
			</span>
