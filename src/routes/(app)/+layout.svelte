	...
	onMount(async () => {
		...
		try {
			...
			try {
				...
				console.log('[BILLING] Checkout params check:', { checkoutSuccess, sessionId, purchaseId, hasToken: !!authToken, url: window.location.href });
				if (checkoutSuccess === '1' && (sessionId || purchaseId) && authToken) {
					console.log('[BILLING] Confirming checkout session...');
					const confirmRes = await confirmCheckoutSession(
						{ session_id: sessionId, purchase_id: purchaseId },
						authToken
					);
					console.log('[BILLING] Confirm result:', confirmRes);
					try {
				...
				// Check if we should open settings modal (e.g., returning from Stripe setup)
