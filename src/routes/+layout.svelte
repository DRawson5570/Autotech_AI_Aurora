			...
			if ($config) {
				...
				if (localStorage.token) {
					...
					if (sessionUser) {
						...
						// Use user-specific display name for normal users if provided by the backend
						const refreshedConfig = await getBackendConfig();
						if (refreshedConfig?.user_name && sessionUser?.role === 'user') {
							await WEBUI_NAME.set(refreshedConfig.user_name);
						} else {
							await WEBUI_NAME.set(refreshedConfig.name);
						}
					} else {
...
{#if showRefresh && $user?.role === 'admin'}
	<div class=" py-5">
