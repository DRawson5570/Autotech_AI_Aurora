...
# Deploy this repo to poweredge2 by rsync'ing into /prod/autotech_ai and restarting the systemd service.
...
# - The remote host uses systemd service (default: open-webui-local)
#
# Typical usage:
#   ./scripts/deploy-poweredge2.sh --user YOUR_SSH_USER --wipe-data
...
if [[ "${SERVICE_NAME}" == "auto" ]]; then
  if [[ "${DRY_RUN}" == true ]]; then
    echo "Skipping service auto-detect in dry-run; will use 'open-webui-local' for preview."
    SERVICE_NAME=open-webui-local
  else
    ...
    if [[ -n "${detected}" ]]; then
      ...
      echo "WARNING: Could not auto-detect service name on remote. Defaulting to open-webui-local." >&2
      SERVICE_NAME=open-webui-local
    fi
  fi
fi
